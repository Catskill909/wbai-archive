...
| 5 | The studio, phase 4 (actions) | S | Phases 1–3 and 5 are built: the gate, the stats dashboard, operational health and privacy-first usage counters. What remains is the only phase with *write* operations — force a feed re-harvest, refresh the program directory, re-probe the stream, clear a cache — which needs a CSRF token and confirmations. Full design in [admin-page.md](admin-page.md). |
